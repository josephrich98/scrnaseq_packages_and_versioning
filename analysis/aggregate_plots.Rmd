...
for (set in variable_sets) {
    ...
    ggsave(output_plot_filepath, plot = p, dpi = 350, bg = "white", width = 2100, height = 2100, units = "px")
...
statistic_names_total <- c("Cells jaccard", "Genes jaccard", "HVGs jaccard", "Mean ΔPC1-3 loadings", "Median log SNN degree ratio", "ARI", "Median jaccard of UMAP KNN", "Marker Genes Jaccard", "Markers Jaccard", "logFC CCC", "Adj p-value flip rate")
...
df <- data.frame(
    ...
    group = factor(rep(c("Seurat", "Scanpy", "Seurat vs. Scanpy"), each = 10), levels = c("Seurat", "Scanpy", "Seurat vs. Scanpy")),
...
custom_legend <- ggplot(df, aes(x, y, group = group, color = group, linetype = group)) +
    ...
    scale_color_manual(values = c("Seurat" = "#D55E00", "Scanpy" = "#56B4E9", "Seurat vs. Scanpy" = "black")) +
    scale_linetype_manual(values = c("Seurat" = "solid", "Scanpy" = "solid", "Seurat vs. Scanpy (defaults)" = "dashed")) +
    theme(
