...
variable_sets <- list(
  ...
  list(
    statistic_name = "Median log(KNN degree ratio)",
    file_name = "pca_knn_clustering_umap_stats.txt",
    extraction_phrase = "Median log degree ratio of KNN: ",
...
for (set in variable_sets) {
    ...
    #   frac = c(0.02, 0.04, 0.08, 0.16, 0.32, 0.64, 1),
    #   value_group1 = c(.01,.01,.01,.01,.01,.5,1),
    #   value_group2 = c(.1,.1,.1,.1,.1,.1,.1)
    # )
    # baseline_value_seurat_vs_scanpy <- 1
    ...
    p <- ggplot(long_df, aes(x = frac, y = value, group = variable, color = variable)) +
        ...
        scale_y_continuous(limits = c(0, y_axis_max),            # Setting y-axis limits from 0 to 1
                         breaks = seq(0, y_axis_max, by = 0.2), # Major ticks every 0.2
                         minor_breaks = seq(0.1, 0.9, by = 0.2)) + # Minor ticks at 0.1, 0.3, 0.5, 0.7, 0.9
        scale_x_continuous(
    ...
    if (set$statistic_name %in% c("Mean ΔPC1-3 loadings", "Median log(KNN degree ratio)", "Median magnitude ΔlogFC")) {
    ...
    if (!(set$statistic_name %in% c("Markers Jaccard", "Median magnitude ΔlogFC", "Fraction of p-value adj that flipped across 0.05 threshold"))) {
...
statistic_names_total <- c("Cells jaccard", "Genes jaccard", "HVGs jaccard", "Mean ΔPC1-3 loadings", "Median log(KNN degree ratio)", "ARI", "Median jaccard of UMAP KNN", "Marker Genes Jaccard", "Markers Jaccard", "Median magnitude ΔlogFC", "Fraction of p-value adj that flipped across 0.05 threshold")

lines <- readLines(output_stat_filepath)

for (statistic in statistic_names_total) {
    if (statistic %in% c("Markers Jaccard", "Median magnitude ΔlogFC", "Fraction of p-value adj that flipped across 0.05 threshold")) {
...
aggregate_bar_chart <- ggplot(long_bar_chart, aes(x = Category, y = value, fill = variable)) +
    ...
    theme(
        ...
        axis.title = element_text(size = rel(1.45)))  # Rotate x-axis labels if needed
...
```{r}
custom_legend2 <- custom_legend +
    geom_hline(aes(yintercept = 3), size = 0.3)
