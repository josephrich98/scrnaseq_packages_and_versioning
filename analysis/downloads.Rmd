...
```{r}
library(reticulate)
# Sys.setenv(PATH = paste("/home/miniconda/bin", Sys.getenv("PATH"), sep=":"))
# conda_binary <- "/home/miniconda/bin/conda"  # replace with "auto" for default
conda_binary <- "auto"
...
if (!(conda_env_name %in% conda_list(conda_binary)$name)) {
    ...
    # # Do this in place of below if environment.yml exists
...
```{r}
# # # # conda_install(conda = conda_binary,envname = conda_env_name, channel = "conda-forge", c("umap-learn==0.5.4"))   # BAD
# conda_install(conda = conda_binary,envname = conda_env_name, pip = TRUE, c("umap-learn==0.5.2, louvain==0.8.1"))
```