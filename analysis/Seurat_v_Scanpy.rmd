...
output_data_file_paths <- list(
    seu_clusters = glue::glue("{output_base_path}/data_files/seu_clusters.csv"),
...
if (analysis_methods == "default" || analysis_methods == "scanpy_like") {
    ...
    paste(analysis_methods, "is not a valid input for analysis_methods. Please choose from 'default' or 'seurat_like'.")
}
...
if (analysis_methods == "default" || analysis_methods == "seurat_like") {
    ...
    paste(analysis_methods, "is not a valid input for analysis_methods. Please choose from 'default' or 'scanpy_like'.")
}
```
...
```{r}
# sc_mito_genes <- py$sc_mito_genes
...
```{python}
# overlapping_inds = list(set(adata.obs_names) & set(r.seu_inds))
# overlapping_genes = list(set(adata.var_names) & set(r.seu_genes))
...
Sometimes this cell must be run manually when scanpy_hvg_flavor == "seurat_v3"
```{python}
sc.pp.highly_variable_genes(adata, n_top_genes = n_top_genes, flavor = r.scanpy_hvg_flavor)
...
mat_py <- py$adata$raw$X
mat_py <- as(t(mat_py), "CsparseMatrix")
mat_r <- GetAssayData(seu, "RNA")

inds <- rowSums(mat_r) > 0
# seu <- seu[inds,]
```
...
eigs_df <- tibble(Seurat = var_explained,
...
df_loadings <- make_pc_diffs_df(list(Seurat = pca_loadings_seu,
                            Scanpy = pca_loadings_py), npcs = 50)
...
mylist <- list(Seurat = pca_loadings_seu,
...
if (data_input == "scanpy" && identical(seu_inds, scan_inds)) {
    ...
    # seu[["pca"]]@feature.loadings <- pca_loadings_py
    # var_explained <- py$adata$uns[["pca"]][["variance_ratio"]]
    # seu@reductions$pca@misc$total.variance <- sum(py$adata$uns[["pca"]][["variance"]])
...
if (data_input == "seurat" && identical(seu_inds, scan_inds)) {
    ...
    # pca_loadings_seu <- Loadings(seu, reduction = "pca")
    # ordered_loadings_seu <- pca_loadings_seu[match(hvgs$Scanpy, rownames(pca_loadings_seu)), ]
    # py$adata$varm["PCs"] <- ordered_loadings_seu
    # py$adata$uns[["pca"]][["variance_ratio"]] <- Stdev(seu, reduction = "pca")^2/tot_variance
    # var_explained_py = py$adata$uns[["pca"]][["variance_ratio"]]
}
```
...
nei_sizes <- tibble(Seurat = lengths(seu_list),
...
If data_input == "seurat": Apply SNN graph from Seurat to Scanpy
```{python}
...
# if (data_input == "seurat") {
#     seu_clusters_dataframe <- data.frame(cell_name = seu_inds, cluster = seu_clusters)
#     write.csv(seu_clusters_dataframe, output_data_file_paths$seu_clusters, row.names = FALSE)
# }
```
...
if data_input == "seurat" and (r.seu_inds == r.scan_inds):
    ...
    # seu_clusters = pd.read_csv(r.output_data_file_paths['seu_clusters'])
    # adata.obs = adata.obs.merge(seu_clusters, on='cell_name', how='left')
    # adata.obs.set_index('cell_name', inplace=True)
    # adata.obs[r.scanpy_clustering_algorithm] = pd.Categorical(adata.obs['cluster'])
...
# seu_umap_knn <- BiocNeighbors::findKNN(seu_umap_data, k = umap_knn_k)
# scan_umap_knn <- BiocNeighbors::findKNN(scan_umap_data, k = umap_knn_k)
# 
# seu_umap_scan_clusters_concordex_ratio <- concordexR::calculateConcordex(seu_umap_knn$index, labels = scan_cluster_data_original, k = umap_knn_k, n.iter = 100)
# 
# scan_umap_seu_clusters_concordex_ratio <- concordexR::calculateConcordex(scan_umap_knn$index, labels = seu_cluster_data_original, k = umap_knn_k, n.iter = 100)
# 
# 
# all_seu <- concordexR::calculateConcordex(seu_umap_knn$index, labels = seu_cluster_data_original, k = umap_knn_k, n.iter = 100)
# 
# all_scan <- concordexR::calculateConcordex(scan_umap_knn$index, labels = scan_cluster_data_original, k = umap_knn_k, n.iter = 100)
# 
# 
# if(file_paths$pca_knn_clustering_umap_file != FALSE) {
#     sink(file_paths$pca_knn_clustering_umap_file, append = TRUE, split = TRUE)
# }
# 
# print(paste("Concordex ratio for Seu UMAP with Scan clusters: ", seu_umap_scan_clusters_concordex_ratio$concordex_ratio))
# print(paste("Concordex ratio for Scan UMAP with Seu clusters: ", scan_umap_seu_clusters_concordex_ratio$concordex_ratio))
# print(paste("All seu: ", all_seu$concordex_ratio))
# print(paste("All scan: ", all_scan$concordex_ratio))
# print(paste("Concordex ratio for Seu UMAP with Scan clusters, normalized by perfect Seu concordance: ", seu_umap_scan_clusters_concordex_ratio$concordex_ratio / all_seu$concordex_ratio))
# print(paste("Concordex ratio for Scan UMAP with Seu clusters, normalized by perfect Scan concordance: ", scan_umap_seu_clusters_concordex_ratio$concordex_ratio / all_scan$concordex_ratio))
# 
# if(file_paths$pca_knn_clustering_umap_file != FALSE) {
#     sink()
# }
...
df_umap <- tibble(Seurat = seu_umap_knn_clusters,
...
umap_alluvial_plot_legend <- plot_alluvial(clus_df_gather_umap, color_boxes = TRUE, color_bands = TRUE, alluvial_alpha = 0.5, save = file_paths$umap_alluvial)
...
```{r}
umap_plots <- plot_umap(group1_umap_info = seu_umap_data, group1_clusters = seu_umap_knn_clusters, group2_umap_info = scan_umap_data, group2_clusters = scan2_clusters_renumbered_umap, group1 = "Seurat", group2 = "Scanpy", save = c(file_paths$umap_umap_leiden_seu, file_paths$umap_umap_leiden_scan))
seu_umap <- umap_plots[[1]]
scan_umap <- umap_plots[[2]]
...
if (!file.exists(output_data_file_paths$markers)) {
    if (analysis_methods == "default" || analysis_methods == "seurat_like") {
        ...
        markers <- FindAllMarkers(seu, logfc.threshold=0, min.pct=0, return.thresh=1.0001)
...
Scanpy manual filtering
```{r}
# min_logFC <- min(abs(result$log_fc), na.rm = TRUE)   # log_fc or avg_log2FC - should be 0.25
# max_p_val <- max(result$p_value, na.rm = TRUE)   # p_value or p_val - should be 0.01
# exists_row <- any(result$pts < 0.1 & result$pts_rest < 0.1)  # should be FALSE
# 
# min_logFC
# max_p_val
# exists_row
# cat("Final number of rows in unfiltered:", nrow(result), "\n")
...
# vectorized_scan_unfiltered_markers <- unique(result$gene)
...
# make_euler_seurat_vs_scanpy(vectorized_seu_unfiltered_markers, vectorized_scan_unfiltered_markers, comparison="Marker", save_plot = file_paths$euler_after_qc_marker_genes_only, save_stats = file_paths$de_stats_file)
...
markers2 <- markers2 |> 
    dplyr::rename(p_val_r = p_val, logFC_r = avg_log2FC, p_val_adj_r = p_val_adj,
...
# markers2 |>
#     mutate(is0 = near(pct.1, 0) | near(pct.2, 0)) |>
#     # filter(!is0) |>
#     filter(abs(logFC_r) < 15) |>
#     filter(abs(logFC_py) < 15) |>
#     arrange(desc(logFC_py)) -> markers2_outliers_removed
# 
# seurat_vs_scanpy_logFC_scatterplot_outliers_removed <- plot_scatterplot_de(markers2_outliers_removed, save = file_paths$logFC_scatterplot_outliers_removed_file_path, outliers_excluded = TRUE, show_legend = TRUE)
# seurat_vs_scanpy_pvaladj_scatterplot_outliers_removed <- plot_scatterplot_de(markers2_outliers_removed, save = file_paths$wilcoxon_scatterplot_outliers_removed_file_path, outliers_excluded = TRUE)
...
# seurat_vs_scanpy_logFC_scatterplot_outliers_removed
# seurat_vs_scanpy_pvaladj_scatterplot_outliers_removed
```


FC histogram (unlogged)
```{r}
# source(glue("{project_base_path}/scripts/plotting_and_stats.R"))

# seurat_vs_scanpy_FC_histogram_magnitude <- plot_differences_histogram_seurat_vs_scanpy(markers2, "FC_difference_magnitude", title = glue("Histogram of ΔFC (|Seu-Scan|)"), x_label = "ΔFC (|Seu-Scan|)", save = file_paths$FC_histogram_magnitude_file_path)
# seurat_vs_scanpy_FC_histogram_signed <- plot_differences_histogram_seurat_vs_scanpy(markers2_outliers_removed, "FC_difference_signed", title = glue("Histogram of ΔFC (Seu-Scan)"), x_label = "ΔFC (Seu-Scan)", save = file_paths$FC_histogram_signed_file_path)

# seurat_vs_scanpy_FC_histogram_magnitude
# seurat_vs_scanpy_FC_histogram_signed
```
