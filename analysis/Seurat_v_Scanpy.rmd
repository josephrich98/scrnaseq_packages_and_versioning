...
output_base_path <- "/workspace/test1"   # NULL for default path
...
if (include_seeds_in_file_paths) {
    if (seu_read_fraction_after_downsampling == scan_read_fraction_after_downsampling) {
        read_fraction_after_downsampling <- glue::glue("{read_fraction_after_downsampling}_seed{seu_read_fraction_after_downsampling}")
    } else {
        read_fraction_after_downsampling <- glue::glue("{read_fraction_after_downsampling}_seu1_seed{seu_read_fraction_after_downsampling}_seu2_seed{scan_read_fraction_after_downsampling}")
    }
...
```{r}
source(glue("{project_base_path}/scripts/plotting_and_stats.R"))
upset_cell <- make_upset_seurat_vs_scanpy(seu, py$adata, comparison = "Cell", save = "/workspace/my_upset.tiff")
upset_gene <- make_upset_seurat_vs_scanpy(seu, py$adata, comparison = "Gene", save = "/workspace/my_upset2.tiff")  # file_paths$upset_genes
```
