...
if (seu_matrix_qc == "raw") {
    if (seu_matrix_generation_method == "kb") {
        res_mat <- read_count_output_modified(seu_data_path, name = "cells_x_genes", unspliced = seu_matrix_include_unspliced, batch = seu_used_batch, tcc = FALSE)
...
if r.scan_matrix_qc == "raw":  
    if (scan_matrix_generation_method=="kb" and r.scan_matrix_include_unspliced):
        ...
        if r.scan_used_batch:
            adata = kb_utils.import_matrix_as_anndata(f"{scan_data_path}/{matrix_file}",f"{scan_data_path}/cells_x_genes.barcodes.combined.txt",f"{scan_data_path}/cells_x_genes.genes.txt")
        else:
...
#* erase
pca12_plot_TR <- plot_pca_compare(pca_embeddings1, pca_embeddings2, legend_position = "TR", group_labels = unlist(seurat_group_names), save = glue::glue("{output_base_path}/plots/pca_scatterplot_12_TR.tiff"))
pca12_plot_TL <- plot_pca_compare(pca_embeddings1, pca_embeddings2, legend_position = "TL", group_labels = unlist(seurat_group_names), save = glue::glue("{output_base_path}/plots/pca_scatterplot_12_TL.tiff"))
pca12_plot_BR <- plot_pca_compare(pca_embeddings1, pca_embeddings2, legend_position = "BR", group_labels = unlist(seurat_group_names), save = glue::glue("{output_base_path}/plots/pca_scatterplot_12_BR.tiff"))
pca12_plot_BL <- plot_pca_compare(pca_embeddings1, pca_embeddings2, legend_position = "BL", group_labels = unlist(seurat_group_names), save = glue::glue("{output_base_path}/plots/pca_scatterplot_12_BL.tiff"))
```
...
```{python}
# adata = anndata.read_h5ad(r.adata_path)
# snn_graph_scan = adata.obsp['connectivities']
```
