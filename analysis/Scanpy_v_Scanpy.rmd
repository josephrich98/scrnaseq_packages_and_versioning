...
scan1_kb_major_version <- as.integer(sub("^[0-9]+_([0-9]+)_.*$", "\\1", scan1_matrix_generation_method_version))
scan2_kb_major_version <- as.integer(sub("^[0-9]+_([0-9]+)_.*$", "\\1", scan2_matrix_generation_method_version))
...
if r.scan1_matrix_qc == "raw":
    if (scan1_matrix_generation_method=="kb" and r.scan1_matrix_include_unspliced):
        ...
        if r.scan1_used_batch:
            adata1 = kb_utils.import_matrix_as_anndata(f"{scan1_data_path}/{matrix_file}",f"{scan1_data_path}/cells_x_genes.barcodes.combined.txt",f"{scan1_data_path}/cells_x_genes.genes.txt")
        else:
...
if r.scan2_matrix_qc == "raw":
    if (scan2_matrix_generation_method=="kb" and r.scan2_matrix_include_unspliced):
        matrix_file = "cells_x_genes.total.mtx"
        if r.scan2_used_batch:
            adata2 = kb_utils.import_matrix_as_anndata(f"{scan2_data_path}/{matrix_file}",f"{scan2_data_path}/cells_x_genes.barcodes.combined.txt",f"{scan2_data_path}/cells_x_genes.genes.txt")
        else:
...
sc.pp.regress_out(adata1, ['total_counts', 'pct_mt'])

adata2 = adata2[:, adata2.var.highly_variable]
sc.pp.regress_out(adata2, ['total_counts', 'pct_mt'])
```
...
#* erase
pca12_plot_TR <- plot_pca_compare(pca_embeddings1, pca_embeddings2, group1_name = "Scanpy1", group2_name = "Scanpy2", legend_position = "TR", group_labels = unlist(seurat_group_names), save = glue::glue("{output_base_path}/plots/pca_scatterplot_12_TR.tiff"))
pca12_plot_TL <- plot_pca_compare(pca_embeddings1, pca_embeddings2, group1_name = "Scanpy1", group2_name = "Scanpy2", legend_position = "TL", group_labels = unlist(seurat_group_names), save = glue::glue("{output_base_path}/plots/pca_scatterplot_12_TL.tiff"))
pca12_plot_BR <- plot_pca_compare(pca_embeddings1, pca_embeddings2, group1_name = "Scanpy1", group2_name = "Scanpy2", legend_position = "BR", group_labels = unlist(seurat_group_names), save = glue::glue("{output_base_path}/plots/pca_scatterplot_12_BR.tiff"))
pca12_plot_BL <- plot_pca_compare(pca_embeddings1, pca_embeddings2, group1_name = "Scanpy1", group2_name = "Scanpy2", legend_position = "BL", group_labels = unlist(seurat_group_names), save = glue::glue("{output_base_path}/plots/pca_scatterplot_12_BL.tiff"))
```
