...
seu1_read_fraction_after_downsampling <- "1.0" # fraction of reads after downsampling - any number from (0,1.0]
seu2_read_fraction_after_downsampling <- "1.0" # fraction of reads after downsampling - any number from (0,1.0]

seu1_cell_fraction_after_downsampling <- "1.0" # fraction of cells after downsampling - any number from (0,1.0]
seu2_cell_fraction_after_downsampling <- "1.0" # fraction of cells after downsampling - any number from (0,1.0]

# Matrix generation methods
seu1_matrix_generation_method <- "cellranger" # str["kb", "cellranger"]
seu1_matrix_generation_method_version <- "7.2.0"
seu1_matrix_include_unspliced <- TRUE # kb only; if TRUE --> .total.mtx (kb SUA); if FALSE --> .cell.mtx (KB SA) or .mtx (older versions of KB)
seu1_matrix_source <- "generated" # "generated" (kb/cellranger count) or "downloaded" (eg 10x website)
seu1_matrix_qc <- "raw" # "raw" or "filtered"
seu1_used_batch <- FALSE

seu2_matrix_generation_method <- "cellranger" # str["kb", "cellranger"]
seu2_matrix_generation_method_version <- "3.1.0"
seu2_matrix_include_unspliced <- FALSE # kb only; if TRUE --> .total.mtx (kb SUA); if FALSE --> .cell.mtx (KB SA) or .mtx (older versions of KB)
seu2_matrix_source <- "generated" # "generated" (kb/cellranger count) or "downloaded" (eg 10x website)
seu2_matrix_qc <- "raw" # "raw" or "filtered"
seu2_used_batch <- FALSE
...
seu1_name <- "Cellranger7"
seu2_name <- "Cellranger3"
...
seu1_read_fraction_after_downsampling <- gsub("\\.", "_", as.character(seu1_read_fraction_after_downsampling)) # fraction of reads after downsampling, as string representation and using underscores in place of decimal
seu2_read_fraction_after_downsampling <- gsub("\\.", "_", as.character(seu2_read_fraction_after_downsampling))
seu1_cell_fraction_after_downsampling <- gsub("\\.", "_", as.character(seu1_cell_fraction_after_downsampling))
seu2_cell_fraction_after_downsampling <- gsub("\\.", "_", as.character(seu2_cell_fraction_after_downsampling))

seu1_matrix_generation_method_version <- gsub("\\.", "_", seu1_matrix_generation_method_version)
seu2_matrix_generation_method_version <- gsub("\\.", "_", seu2_matrix_generation_method_version)

seu1_included_transcripts <- ifelse(seu1_matrix_include_unspliced == TRUE, "sua", "sa")
seu2_included_transcripts <- ifelse(seu2_matrix_include_unspliced == TRUE, "sua", "sa")

seu1_matrix_generation_method_full <- glue::glue("{seu1_matrix_generation_method}{seu1_matrix_generation_method_version}_{seu1_matrix_qc}_{seu1_matrix_source}")
seu2_matrix_generation_method_full <- glue::glue("{seu2_matrix_generation_method}{seu2_matrix_generation_method_version}_{seu2_matrix_qc}_{seu2_matrix_source}")

seu1_kb_major_version <- as.integer(sub("^[0-9]+_([0-9]+)_.*$", "\\1", seu1_matrix_generation_method_version))
seu2_kb_major_version <- as.integer(sub("^[0-9]+_([0-9]+)_.*$", "\\1", seu2_matrix_generation_method_version))
...
Data paths
```{r}
seu1_data_path <- glue::glue("{project_base_path}/count_matrix_collection/{data_name}/{seu1_matrix_generation_method_full}/frac{seu1_read_fraction_after_downsampling}_seed{seu1_read_downsample_seed}")
seu2_data_path <- glue::glue("{project_base_path}/count_matrix_collection/{data_name}/{seu2_matrix_generation_method_full}/frac{seu2_read_fraction_after_downsampling}_seed{seu2_read_downsample_seed}")
```

File path definitions
```{r}
if (seu1_matrix_generation_method == seu2_matrix_generation_method &&
    seu1_matrix_generation_method_version == seu2_matrix_generation_method_version &&
    seu1_included_transcripts == seu2_included_transcripts &&
    seu1_matrix_source == seu2_matrix_source &&
    seu1_matrix_qc == seu2_matrix_qc) {
    matrix_generation_method_full <- glue::glue("{seu1_matrix_generation_method_full}_{seu1_included_transcripts}")
    # Full vs downsampled
    group1_color <- "#FFCB57"
    group2_color <- "#6C27CC"
} else {
    matrix_generation_method_full <- glue::glue("seu1_{seu1_matrix_generation_method_full}_{seu1_included_transcripts}_vs_seu2_{seu2_matrix_generation_method_full}_{seu2_included_transcripts}")
    # New vs old version
    group1_color <- "#009E73"
    group2_color <- "#CC79A7"
}
...
if (include_seeds_in_file_paths) {
    read_fraction_after_downsampling <- glue::glue("{read_fraction_after_downsampling}_seu1_seed{seu1_read_downsample_seed}_seu2_seed{seu2_read_downsample_seed}")
...
```{r}
if (seu1_matrix_qc == "raw") {
    if (seu1_matrix_generation_method == "kb") {
        res_mat1 <- read_count_output_modified(seu1_data_path, name = "cells_x_genes", unspliced = seu1_matrix_include_unspliced, batch = seu1_used_batch, tcc = FALSE)
...
} else if (seu1_matrix_qc == "filtered") {
    seu1 <- CreateSeuratObject(Read10X(seu1_data_path, gene.column = 1), min.cells = seu1_min_cells, min.features = seu1_min_features)
} else {
...
```{r}
if (seu2_matrix_qc == "raw") {
    if (seu2_matrix_generation_method == "kb") {
        res_mat2 <- read_count_output_modified(seu2_data_path, name = "cells_x_genes", unspliced = seu2_matrix_include_unspliced, batch = seu2_used_batch, tcc = FALSE)
...
} else if (seu2_matrix_qc == "filtered") {
    seu2 <- CreateSeuratObject(Read10X(seu2_data_path, gene.column = 1), min.cells = seu2_min_cells, min.features = seu2_min_features)
} else {
    stop("Invalid value for seu_matrix_qc. It should be either 'raw' or 'filtered'.")
}
```
...
#* erase
pca12_plot_TR <- plot_pca_compare(pca_embeddings1, pca_embeddings2, group1_name = "Seurat1", group2_name = "Seurat2", legend_position = "TR", group_labels = unlist(seurat_group_names), save = glue::glue("{output_base_path}/plots/pca_scatterplot_12_TR.tiff"))
pca12_plot_TL <- plot_pca_compare(pca_embeddings1, pca_embeddings2, group1_name = "Seurat1", group2_name = "Seurat2", legend_position = "TL", group_labels = unlist(seurat_group_names), save = glue::glue("{output_base_path}/plots/pca_scatterplot_12_TL.tiff"))
pca12_plot_BR <- plot_pca_compare(pca_embeddings1, pca_embeddings2, group1_name = "Seurat1", group2_name = "Seurat2", legend_position = "BR", group_labels = unlist(seurat_group_names), save = glue::glue("{output_base_path}/plots/pca_scatterplot_12_BR.tiff"))
pca12_plot_BL <- plot_pca_compare(pca_embeddings1, pca_embeddings2, group1_name = "Seurat1", group2_name = "Seurat2", legend_position = "BL", group_labels = unlist(seurat_group_names), save = glue::glue("{output_base_path}/plots/pca_scatterplot_12_BL.tiff"))
```
...
if (data_input == "seu1" && identical(seu1_inds, seu2_inds)) {
    seu2[["pca"]] <- CreateDimReducObject(embeddings = as.matrix(pca_embeddings1), key = "PC_", assay = DefaultAssay(seu1))
    seu2[["pca"]]@feature.loadings <- pca_loadings_seu1
}

if (data_input == "seu2" && identical(seu1_inds, seu2_inds)) {
    seu1[["pca"]] <- CreateDimReducObject(embeddings = as.matrix(pca_embeddings2), key = "PC_", assay = DefaultAssay(seu2))
...
print(glue("Median magnitude of log degree ratio of SNN: {median_logged_degree_ratio}"))
...
if (file_paths$pca_cluster_filepath_seu1 != FALSE) {
    ggsave(file_paths$pca_cluster_filepath_seu1, plot = new_cluster_pca1, dpi = dpi)
}

if (file_paths$pca_cluster_filepath_seu2 != FALSE) {
    ggsave(file_paths$pca_cluster_filepath_seu2, plot = new_cluster_pca2, dpi = dpi)
}
```
...
Plot UMAP
```{r}
umap_plots <- plot_umap(group1_umap_info = seu1_umap_info$umap@cell.embeddings, group1_clusters = seu1$seurat_clusters, group2_umap_info = seu2_umap_info$umap@cell.embeddings, group2_clusters = seu2_clusters_renumbered, group1 = seu1_name, group2 = seu2_name, save = c(file_paths$umap_seu1, file_paths$umap_seu2))
...
if (identical(seu1_inds, seu2_inds)) {
    umap_plots_swapped_clusters <- plot_umap(group1_umap_info = seu1_umap_info$umap@cell.embeddings, group1_clusters = seu2_clusters_renumbered, group2_umap_info = seu2_umap_info$umap@cell.embeddings, group2_clusters = seu1$seurat_clusters, group1 = seu1_name, group2 = seu2_name, group1_title = glue("Seurat {seu1_name} UMAP with {seu2_name} clusters"), group2_title = glue("Seurat {seu2_name} UMAP with {seu1_name} clusters"), save = c(file_paths$umap_seu1_clusters_seu2, file_paths$umap_seu2_clusters_seu1))
...
```{r}
umap_plots <- plot_umap(group1_umap_info = seu1_umap_data, group1_clusters = seu1_umap_knn_clusters, group2_umap_info = seu2_umap_data, group2_clusters = seu2_clusters_renumbered_umap, group1 = seu1_name, group2 = seu2_name, save = c(file_paths$umap_umap_leiden_seu1, file_paths$umap_umap_leiden_seu2))
seu1_umap <- umap_plots[[1]]
seu2_umap <- umap_plots[[2]]
...
markers2 <- markers_seu1 |>
    inner_join(markers_seu2, by = c("cluster", "gene"), suffix = c(glue(".{seu1_name}"), glue(".{seu2_name}"))) # ? originally left join
...
```{r}
data_pca <- data.frame(Cellranger7 = markers2$avg_log2FC.Cellranger7, 
                       Cellranger3 = markers2$avg_log2FC.Cellranger3)
...
intercept <- data_mean[2] - slope * data_mean[1]

p <- ggplot(data_pca, aes(x = Cellranger7, y = Cellranger3)) +
    geom_point() +
    labs(x = "Average log2 FC Cellranger 7", y = "Average log2 FC Cellranger 3", title = "PCA Fit Line with PC1") +
    theme_minimal() + 
