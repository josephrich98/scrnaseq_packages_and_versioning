...
seu1_name <- "Cellrangerv7"
seu2_name <- "Cellrangerv3"
...
file_paths <- list(
    ...
    umap_seu1_clusters_seu2 = glue::glue("{output_base_path}/plots/umap_seu_{seu1_name}_clusters_{seu2_name}.tiff"),
    umap_seu2_clusters_seu1 = glue::glue("{output_base_path}/plots/umap_seu_{seu2_name}_clusters_{seu1_name}.tiff"),
...
QC and filter high mito cells out of Seurat1, Violin plots
```{r}
...
QC and filter high mito cells out of Seurat2, Violin plots
```{r}
...
eigs_df <- tibble(Seurat1 = var_explained1,
...
df_loadings <- make_pc_diffs_df(list(Seurat1 = pca_loadings_seu1,
                            Seurat2 = pca_loadings_seu2), npcs = 50)
...
mylist <- list(Seurat1 = pca_loadings_seu1,
...
if (data_input == "seu1" && identical(seu1_inds, seu2_inds)) {
    ...
    # Stdev(seu2, reduction = "pca") <- Stdev(seu1, reduction = "pca")
}
...
if (data_input == "seu2" && identical(seu1_inds, seu2_inds)) {
    ...
    # Stdev(seu1, reduction = "pca") <- Stdev(seu2, reduction = "pca")
}
```
...
nei_sizes <- tibble(Seurat1 = lengths(seu1_list),
...
df <- tibble(Seurat1 = seu1_clusters,
...
df_umap <- tibble(Seurat1 = seu1_umap_knn_clusters,
...
# umap_alluvial_plot <- plot_alluvial(clus_df_gather_umap, color_boxes = TRUE, color_bands = FALSE, group1_name = seu1_name, group2_name = seu2_name, save = file_paths$alluvial)
umap_alluvial_plot_legend <- plot_alluvial(clus_df_gather_umap, color_boxes = TRUE, color_bands = TRUE, alluvial_alpha = 0.5, group1_name = seu1_name, group2_name = seu2_name, save = file_paths$umap_alluvial)
...
Now add the results to the Seurat data frame
```{r}
