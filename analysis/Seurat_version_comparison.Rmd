...
seu1_markers_data_path <- "/workspace/analysis/output/SC3_v3_NextGem_SI_PBMC_10K/seuratv4.3.0_vs_scanpyv1.9.5/methods_default/input_default/kb0_28_0_raw_generated_sa/cell_fraction_1_0/read_fraction_1_0/data_files/markers.rds"   # "/workspace/analysis/output/SC3_v3_NextGem_SI_PBMC_10K/seuratvPLEASE_WORK_OLD_ARGUMENTS/input_default/kb0_28_0_raw_generated_sa/cell_fraction_1_0/read_fraction_1_0/data_files/markers_seuv5.rds"
seu2_markers_data_path <- "/workspace/analysis/output/SC3_v3_NextGem_SI_PBMC_10K/seuratvPLEASE_WORK_OLD_ARGUMENTS/input_default/kb0_28_0_raw_generated_sa/cell_fraction_1_0/read_fraction_1_0/data_files/markers_seuv5.rds"   # "/workspace/analysis/output/SC3_v3_NextGem_SI_PBMC_10K/seuratv4.3.0_vs_scanpyv1.9.5/methods_default/input_seurat/kb0_28_0_raw_generated_sa/cell_fraction_1_0/read_fraction_1_0/data_files/results_scan.rds"

output_base_path <- glue::glue("{project_base_path}/seurat5 vs seurat4")


# seu1_data_path <- ""
# seu1_markers_data_path <- ""
# seu2_data_path <- ""
# seu2_markers_data_path <- ""
```
...
```{r}
if (as.integer(scanpy_unfiltered) == 1) {
    markers_seu1_unfiltered <- markers_seu1

    markers_seu1 <- markers_seu1 %>%
        filter(!(pts < 0.1 & pts_rest < 0.1)) %>%   # 0.01 in Seu5
        filter(!(abs(log_fc) < 0.25)) %>%  # 0.1 in Seu5
        filter(p_value < 0.01)
} else if (as.integer(scanpy_unfiltered) == 2) {
    markers_seu2_unfiltered <- markers_seu2

    markers_seu2 <- markers_seu2 %>%
        filter(!(pts < 0.1 & pts_rest < 0.1)) %>%   # 0.01 in Seu5
        filter(!(abs(log_fc) < 0.25)) %>%  # 0.1 in Seu5
        filter(p_value < 0.01)
}

if ("log_fc" %in% names(markers_seu1)) {
    markers_seu1 <- markers_seu1 |> dplyr::rename(avg_log2FC = log_fc)
}

if ("p_value_adj" %in% names(markers_seu1)) {
    markers_seu1 <- markers_seu1 |> dplyr::rename(p_val_adj = p_value_adj)
}

if ("log_fc" %in% names(markers_seu2)) {
    markers_seu2 <- markers_seu2 |> dplyr::rename(avg_log2FC = log_fc)
}

if ("p_value_adj" %in% names(markers_seu2)) {
    markers_seu2 <- markers_seu2 |> dplyr::rename(p_val_adj = p_value_adj)
}
...
if (as.integer(scanpy_unfiltered) == 1) {
    markers_seu1 <- markers_seu1_unfiltered
} else if (as.integer(scanpy_unfiltered) == 2) {
    markers_seu2 <- markers_seu2_unfiltered
}
